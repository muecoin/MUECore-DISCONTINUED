...
{
    // return if URI is not valid or is no mue: URI
    if(!uri.isValid() || uri.scheme() != QString("mue"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert mue:// to mue:
    //
    //    Cannot handle this later, because mue:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("mue://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "mue:");
    }
...
{
    QString ret = QString("mue:%1").arg(info.address);
    int paramCount = 0;
...
void openConfigfile()
{
    ...
    /* Open mue.conf with the associated application */
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "mue.desktop";
    return GetAutostartDir() / strprintf("mue-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a mue.desktop file to the autostart directory:
