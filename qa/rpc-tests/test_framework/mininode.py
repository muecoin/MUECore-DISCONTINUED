...
#
# NodeConn: an object which manages p2p connectivity to a mue node
...
# Objects that map to mued objects, which can be serialized/deserialized
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(muehash(r))
            self.hash = encode(muehash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in mued indicates these should be deserialized as blocks
